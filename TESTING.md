...
- `--lock` 验证通过：8 个并发调用只启动一个服务，重复调用在配置/密码读取前静默返回 0；
  锁跨后台 fork 和原终端退出保持有效，执行的远端命令不继承锁；正常退出及 SIGKILL 后可重新启动。
  前台模式、相对锁路径、无效锁路径报错和锁文件与服务文件冲突检查也通过。
...
上一版后台模式构建在 Bob 和 Alice 上分别运行完整进程集成测试及 Unix 后台生命周期测试，均通过。
本次 `--lock` 改动在 macOS 完成完整测试，Linux 版本在 Bob 上通过后台和单实例锁测试。
Alice 的 SSH（端口 2200）主机密钥与本机记录不符，SSH 拒绝连接，本次未复测 Alice；未修改主机密钥信任记录。
...
```text
macOS: fac6c5a8c54f2988112737f380708043cd6974753c9c4ee38f196c4d8e866dc2
Linux: 360dd91d48919f30342e489f239cffce7c4d468c9e30678acc4f2e1d8272037f
```
